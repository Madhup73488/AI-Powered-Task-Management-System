...
import { TaskChatbot } from "@/app/components/TaskChatbot";
...
interface Task {
  ...
  users: {
    ...
    email: string;
    status?: "active" | "inactive";
  } | null; // Added email for search and status
}
...
export default function AdminTasksPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const tasksWithUsers = (tasksData || [])
          .map((task) => {
  ...
  const handleGenerateSummary = async () => {
    ...
    try {
      const response = await fetch("/api/ai/summarize-task", {
        method: "POST",
  ...
  const handleSuggestDeadline = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        suggestedDate.setDate(suggestedDate.getDate() + data.days);
        setNewTaskDeadline(suggestedDate.toISOString().split("T")[0]);
      } else {
        alert(data.error || "Failed to suggest deadline");
      }
    } catch (error) {
      console.error("Error suggesting deadline:", error);
      alert("Failed to suggest deadline");
    } finally {
      setAiDeadlineLoading(false);
    }
  };
  ...
  const handleStatusChange = async (
    taskId: string,
    newStatus: "not_picked" | "in_progress" | "completed",
  ) => {
    try {
  ...
  const handleUpdateTask = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (
        ...
        editingTask.assigned_to !== oldAssignedTo &&
        updatedTaskWithUser.users
      ) {
        // New assignment
        ...
        const changes = [];
        if (editingTask.status !== originalTask?.status)
          changes.push(`status to ${editingTask.status.replace("_", " ")}`);
        if (editingTask.priority !== originalTask?.priority)
          changes.push(`priority to ${editingTask.priority}`);
        if (editingTask.deadline !== originalTask?.deadline)
          changes.push("deadline");
        if (editingTask.title !== originalTask?.title) changes.push("title");
        if (editingTask.description !== originalTask?.description)
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div
          className={`flex justify-center gap-2 ${darkMode ? "text-gray-300" : "text-gray-900"}`}
        >
          <span className="text-4xl animate-[bounce_1s_ease-in-out_0s_infinite]">
            .
          </span>
          <span className="text-4xl animate-[bounce_1s_ease-in-out_0.2s_infinite]">
            .
          </span>
          <span className="text-4xl animate-[bounce_1s_ease-in-out_0.4s_infinite]">
            .
          </span>
        </div>
      </div>
  ...
  return (
    <div className="p-3 md:p-6">
      <Card
        className={`shadow-sm overflow-hidden ${darkMode ? "bg-black border-gray-800" : "bg-white border-blue-100"}`}
      >
        <CardHeader className="p-4 md:p-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 md:gap-4">
            <div>
              ...
              <p
                className={`text-xs md:text-sm mt-1 ${darkMode ? "text-gray-300" : "text-gray-700"}`}
              >
                ...
                <form onSubmit={handleCreateTask} className="grid gap-4 py-4">
                  <div className="grid gap-2">
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button
                        ...
                        disabled={
                          aiSummaryLoading || !newTaskDescription.trim()
                        }
                        title="Generate title from description"
                      >
                        <Sparkles
                          className={`h-4 w-4 ${aiSummaryLoading ? "animate-spin" : ""}`}
                        />
                      </Button>
                    </div>
                  </div>
                  ...
                  <div className="grid gap-2">
                    ...
                    >
                      ...
                      <SelectContent
                        className={`${darkMode ? "bg-gray-900 border-gray-800 text-white" : "bg-white border-gray-200 text-gray-900"}`}
                        style={{
                          backgroundColor: darkMode ? "#111827" : "#ffffff",
                        }}
                      >
                        <SelectItem
                          className={`${darkMode ? "hover:bg-gray-800 focus:bg-gray-800 text-white" : "hover:bg-gray-100 focus:bg-gray-100 text-gray-900"}`}
                          value="not_picked"
                        >
                          Not Picked
                        </SelectItem>
                  ...
                  <div className="grid gap-2">
                    ...
                    <div className="flex gap-2">
                      ...
                      >
                        <Calendar
                          className={`h-4 w-4 ${aiDeadlineLoading ? "animate-spin" : ""}`}
                        />
                      </Button>
                    </div>
                  </div>
        ...
        <CardContent
          className={`p-4 md:p-6 ${darkMode ? "bg-black" : "bg-white"}`}
        >
          {/* Search Bar */}
          <div className="mb-6">
          ...
          <div className="overflow-x-auto">
            <Table>
              <TableHeader>
                <TableRow
                  className={`border-b-2 ${darkMode ? "border-gray-800 bg-gray-900/50" : "border-gray-200 bg-gray-50"}`}
                >
                  <TableHead
              ...
              <TableBody>
                {tasks.map((task) => (
                  <TableRow
                    key={task.id}
                    className={`border-b ${darkMode ? "border-gray-800 hover:bg-gray-800/50" : "border-gray-200 hover:bg-gray-50"}`}
                  >
                    <TableCell
          ...
          {tasks.length === 0 && (
            <div className="text-center py-8 md:py-12 px-4">
              ...
              <h3
                className={`text-sm md:text-base font-semibold mb-2 ${darkMode ? "text-white" : "text-black"}`}
              >
                No tasks found
              </h3>
              <p
                className={`text-xs md:text-sm mb-4 ${darkMode ? "text-slate-300" : "text-black"}`}
              >
                {searchParams.get("search")
                  ? `No tasks match "${searchParams.get("search")}"`
      ...
      {/* AI Chatbot */}
      {currentUser && (
        <TaskChatbot userId={currentUser.id} darkMode={darkMode} />
      )}
    </div>
  );
}
