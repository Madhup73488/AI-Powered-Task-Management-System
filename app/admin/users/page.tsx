...
export default function AdminUsersPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      const activeUsers = (data as User[]).filter(
  ...
  const handleRevokeInvitation = async (invitationId: string) => {
    if (
      !confirm(
        "Are you sure you want to revoke this invitation? This will also delete the associated user account if it exists.",
      )
    )
      return;
    ...
    try {
      ...
      console.log(
        "Client: Deleting invitation from Supabase DB:",
        invitationId,
      );
      ...
      if (deleteInvError) {
        console.error(
          "Client: Error deleting invitation from DB:",
          deleteInvError,
        );
        throw deleteInvError;
      }
      console.log(
        "Client: Invitation deleted from DB. Proceeding to delete user account.",
      );

      // Prepare and log the payload for the API route
      const deleteUserPayload = { email: invitationToRevoke.email };
      console.log(
        "Client: Sending delete user API request with payload:",
        deleteUserPayload,
      );
      ...
      if (!response.ok) {
        ...
        console.error(
          "Client: Delete user API responded with an error status:",
          response.status,
          errorText,
        );
        let errorData;
        try {
          ...
          console.error(
            "Client: Failed to parse error response as JSON:",
            parseError,
          );
          throw new Error(
            `API Error: ${errorText || "Unknown error (failed to parse response)"}`,
          );
        }
  ...
  return (
    <div className="p-3 md:p-6">
      <Card
        className={`${darkMode ? "bg-black border-gray-800" : "bg-white border-blue-100"} shadow-sm`}
      >
        <CardHeader className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 p-4 md:p-6">
          <div>
            ...
            <p
              className={`text-xs md:text-sm ${darkMode ? "text-gray-400" : "text-black"} mt-1`}
            >
          ...
          >
            ...
            <DialogContent
              className={`sm:max-w-[425px] max-h-[90vh] overflow-y-auto ${darkMode ? "bg-gray-900 border-gray-800" : "bg-white border-gray-200"}`}
            >
              <DialogHeader>
                <DialogTitle
        ...
        <CardContent className="p-0 md:p-6">
          <div className="overflow-x-auto">
            <Table>
              <TableHeader>
                <TableRow
                  className={`border-b-2 ${darkMode ? "bg-gray-900/50 border-gray-800" : "bg-gray-50 border-gray-200"}`}
                >
                  <TableHead
              ...
              <TableBody>
                {users.map((user) => (
                  ...
                  >
                    <TableCell
                      className={`text-sm font-medium ${darkMode ? "text-gray-300" : "text-black"}`}
                    >
                    ...
                    <TableCell
                      className={`text-sm font-medium capitalize ${darkMode ? "text-gray-300" : "text-black"}`}
                    >
                      {user.role}
                    </TableCell>
                    <TableCell>
                      <div className="flex items-center gap-2">
                        ...
                        >
                          <SelectTrigger
                            className={`w-[100px] md:w-[140px] text-xs md:text-sm ${darkMode ? "bg-gray-800 border-gray-700 text-white" : "bg-white border-gray-300 text-black"}`}
                          >
      ...
      {invitations.length > 0 && (
        <Card
          className={`${darkMode ? "bg-black border-gray-800" : "bg-white border-blue-100"} shadow-sm mt-4 md:mt-6`}
        >
          <CardHeader className="p-4 md:p-6">
            <CardTitle
              className={`text-lg md:text-xl font-bold ${darkMode ? "text-white" : "text-black"}`}
            >
