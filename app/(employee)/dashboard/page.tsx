  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const inProgress =
          tasksData?.filter((t: any) => t.status === "in_progress").length || 0;
        const completed =
          tasksData?.filter((t: any) => t.status === "completed").length || 0;
        const notPicked =
          tasksData?.filter((t: any) => t.status === "not_picked").length || 0;

        setTaskStats({ total, inProgress, completed, notPicked });

        // Calculate priority stats
        const high =
          tasksData?.filter((t: any) => t.priority === "high").length || 0;
        const medium =
          tasksData?.filter((t: any) => t.priority === "medium").length || 0;
        const low =
          tasksData?.filter((t: any) => t.priority === "low").length || 0;
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-full">
        <div
          className={`flex justify-center gap-2 ${darkMode ? "text-gray-300" : "text-gray-600"}`}
        >
    ...
    <div className="p-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 md:gap-4 mb-4 md:mb-6">
        <div>
          ...
          <p
            className={`text-xs md:text-sm ${darkMode ? "text-gray-400" : "text-gray-600"} mt-1`}
          >
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-3">
          <div className="relative">
            ...
            {showDatePicker && (
              <div
                className={`absolute right-0 mt-2 p-4 border rounded-lg shadow-lg z-50 w-64 ${darkMode ? "bg-gray-900 border-gray-800" : "bg-white border-gray-200"}`}
              >
          ...
          <Button
            className={`text-sm w-full sm:w-auto ${darkMode ? "bg-white text-black hover:bg-gray-100" : "bg-gray-900 text-white hover:bg-gray-800"}`}
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
        {statsCards.map((stat, idx) => (
          <Card
            key={idx}
            className={`${darkMode ? "bg-black border-gray-800" : "bg-white border-gray-200"}`}
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        ...
        <Card
          className={`${darkMode ? "bg-black border-gray-800" : "bg-white border-gray-200"} lg:col-span-2`}
        >
          <CardHeader>
            <CardTitle
            ...
            <CardDescription
              className={`text-xs ${darkMode ? "text-gray-400" : ""}`}
            >
          ...
          <CardContent>
            <div className="space-y-3">
              {/* Graph */}
              <div
                className={`h-64 relative rounded-xl p-4 ${darkMode ? "bg-gray-900" : "bg-gray-50"}`}
              >
                <svg className="w-full h-full" viewBox="0 0 700 220">
                  ...
                  {(() => {
                    ...
                    const maxCount = Math.max(
                      ...completionTrend.map((d) => d.count),
                      1,
                    );
                    const totalTasks = completionTrend.reduce(
                      (sum, d) => sum + d.count,
                      0,
                    );
                    ...
                    if (totalTasks === 0) {
                      const points = completionTrend
                        .map((d, i) => {
                          const x =
                            50 + (i / (completionTrend.length - 1)) * 650;
                          const y = 180;
                          return `${x},${y}`;
                        })
                        .join(" ");
              ...
              <div className="flex justify-between px-2">
                {completionTrend.map((d, i) => {
                  if (i % 5 !== 0 && i !== completionTrend.length - 1)
                    return null;
              ...
              {/* Summary */}
              <div
                className={`pt-3 border-t ${darkMode ? "border-gray-800" : "border-gray-200"}`}
              >
                <div className="flex items-center justify-between">
                  <span
                  ...
                  >
                    {completionTrend.reduce((sum, d) => sum + d.count, 0)} tasks
                    completed
                  </span>
                </div>
                {completionTrend.reduce((sum, d) => sum + d.count, 0) === 0 && (
                  ...
                  >
                    No tasks completed in the last 30 days. Start completing
                    tasks to see your progress!
                  </p>
                )}
              </div>
            </div>
          ...
          <CardContent>
            <div className="space-y-4">
              <div>
                ...
                <div
                  className={`w-full rounded-full h-2 ${darkMode ? "bg-gray-800" : "bg-gray-100"}`}
                >
                  <div
                    className="bg-red-500 h-2 rounded-full"
                    style={{
                      width: `${taskStats.total > 0 ? (priorityStats.high / taskStats.total) * 100 : 0}%`,
                    }}
                  ></div>
                </div>
              </div>
              <div>
                ...
                >
                  <div
                    ...
                    style={{
                      width: `${taskStats.total > 0 ? (priorityStats.medium / taskStats.total) * 100 : 0}%`,
                    }}
                  ></div>
                </div>
              </div>
              <div>
                ...
                >
                  <div
                    ...
                    style={{
                      width: `${taskStats.total > 0 ? (priorityStats.low / taskStats.total) * 100 : 0}%`,
                    }}
                  ></div>
                </div>
              </div>
      ...
      {/* AI Chatbot */}
      {user && <TaskChatbot userId={user.id} darkMode={darkMode} />}
    </div>
  );
}
